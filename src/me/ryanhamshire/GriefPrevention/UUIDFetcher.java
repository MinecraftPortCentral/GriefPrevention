...
class UUIDFetcher {
        ...
        if (GriefPrevention.instance.getServer().getOnlineMode()) {
            ...
            for (int i = 0; i * PROFILES_PER_REQUEST < names.size(); i++) {
                ...
                do {
                    ...
                    String body =
                            JSONArray.toJSONString(names.subList(i * PROFILES_PER_REQUEST, Math.min((i + 1) * PROFILES_PER_REQUEST, names.size())));
                    ...
                    try {
                        ...
                        // in case of error 429 too many requests, pause and
                        // then retry later
                        if (e.getMessage().contains("429")) {
                            retry = true;

                            // if this is the first time we're sending anything,
                            // the batch size must be too big
                            // try reducing it
                            if (i == 0 && PROFILES_PER_REQUEST > 1) {
                                GriefPrevention.AddLogEntry(
                                        "Batch size " + PROFILES_PER_REQUEST + " seems too large.  Looking for a workable batch size...");
                                PROFILES_PER_REQUEST = Math.max(PROFILES_PER_REQUEST - 5, 1);
                            }

                            // otherwise, keep the batch size which has worked
                            // for previous iterations
                            // but wait a little while before trying again.
                            else {
                                GriefPrevention
                                        .AddLogEntry("Mojang says we're sending requests too fast.  Will retry every 30 seconds until we succeed...");
                                Thread.sleep(30000);
                            }
                        } else {
                            throw e;
    ...
    public static UUID getUUIDOf(String name) throws Exception {
        ...
        if (result == null) {
            ...
            // this player will lose his land claim blocks, but claims will stay
            // in place as admin claims
